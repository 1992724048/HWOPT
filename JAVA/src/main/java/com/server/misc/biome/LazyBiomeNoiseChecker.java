...
public final class LazyBiomeNoiseChecker {
	...
	private long getTemperature(Climate.Sampler sampler) {
		if (temperature == null) temperature = Climate.quantizeCoord((float) sampler.temperature().compute(ctx));
		return temperature;
	}
	
	private long getHumidity(Climate.Sampler sampler) {
		if (humidity == null) humidity = Climate.quantizeCoord((float) sampler.humidity().compute(ctx));
		return humidity;
	}
	...
	private long getErosion(Climate.Sampler sampler) {
		if (erosion == null) erosion = Climate.quantizeCoord((float) sampler.erosion().compute(ctx));
		return erosion;
	}
	
	private long getWeirdness(Climate.Sampler sampler) {
		if (weirdness == null) weirdness = Climate.quantizeCoord((float) sampler.weirdness().compute(ctx));
		return weirdness;
	}
	
	private long computeDepth(Climate.Sampler sampler, int y) {
		DensityFunction.SinglePointContext depthCtx = new DensityFunction.SinglePointContext(ctx.blockX(), y, ctx.blockZ());
		return Climate.quantizeCoord((float) sampler.depth().compute(depthCtx));
	}
	
	public boolean matches(Climate.Sampler sampler) {
		BiomeEnvelope env = this.envelopeSelector.getEnvelope(QuartPos.fromBlock(ctx.blockX()), 0, QuartPos.fromBlock(ctx.blockZ()));
	...
	public boolean matches(Climate.Sampler sampler, BiomeNoiseCheckState state, int y) {
		BiomeEnvelope env = this.envelopeSelector.getEnvelope(QuartPos.fromBlock(ctx.blockX()), QuartPos.fromBlock(y), QuartPos.fromBlock(ctx.blockZ()));
	...
	public Climate.TargetPoint toTargetPoint(Climate.Sampler sampler, int y) {
		return new Climate.TargetPoint(getTemperature(sampler), getHumidity(sampler), getContinentalness(sampler), getErosion(sampler), computeDepth(sampler, y), getWeirdness(sampler));
	}
