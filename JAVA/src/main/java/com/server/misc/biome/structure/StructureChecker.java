package com.server.misc.biome.structure;

import com.mojang.datafixers.util.Pair;
import com.server.misc.biome.BiomeEnvelopeSelector;
import com.server.misc.biome.LazyBiomeNoiseChecker;
import com.server.misc.biome.mixin.StructureCheckAccessor;
import com.server.misc.biome.mixin.StructureManagerAccessor;
...
public class StructureChecker {
	static TagKey<Structure> UNINITED_SAFE_STRUCTURES = TagKey.create(Registries.STRUCTURE, Identifier.fromNamespaceAndPath("biome", "uninit_safe"));
	...
	public static Pair<BlockPos, Holder<Structure>> getStructureGeneratingAt(Map<Holder<Structure>, BiomeEnvelopeSelector> pStructureHoldersSet, LevelReader pLevel, StructureManager pStructureManager, boolean pSkipKnownStructures, StructurePlacement pPlacement, ChunkPos pChunkPos, Climate.ParameterList<Holder<Biome>> parameters) {
		...
		Object2IntMap<Structure> structureChunkMap = structureCheckAccessor.getLoadedChunks().get(ChunkPos.pack(pChunkPos.x(), pChunkPos.z()));
		...
		for (var entry : pStructureHoldersSet.entrySet()) {
			...
			if (structureChunkMap != null) {
				...
				if (!biomeChecker.matches(sampler)) continue;
