...
| 实体碰撞优化       | 优化 | 已完成   | 26.1     | 批量 AABB 碰撞检测 (Native C++)                                                      |
| 实体渲染剔除优化   | 优化 | 已完成   | 26.1     | 后台线程异步遮挡检测 + 2×2×2 空间格分组 + Cullable 缓存，支持实体/方块实体/Tick 剔除 |
| 实体 AI 优化       | 优化 | 已完成   | 26.1     | 寻路重算添加 5 tick 冷却                                                             |
| 粒子异步 Tick 优化 | 优化 | 已完成   | 26.1     | ForkJoinPool 并行粒子更新 + 光照缓存（byte 压缩）+ 自旋锁环形队列 + 隔帧节流         |
| 网络数据包压缩     | 优化 | 已完成   | 26.1     | 出站流量为原版的~13%                                                                 |
