...
我们不会盲目使用多线程或 GPU 加速，而是对每项优化结合实际场景进行性能评估，判断是否真正需要这些能力。为了使用而使用只会占满计算资源、降低系统响应，徒劳无益。本模组优先考虑与其他 mod 的兼容性，而非为极限优化而设计。
...
| 实体碰撞优化   | 优化 | 开发中 | 26.1     | 优化原版实体碰撞计算 (满足条件将调用GPU加速)                          |
| 实体渲染优化   | 优化 | 开发中 | 26.1     |                                                                       |
