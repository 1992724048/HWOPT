...
**正在开发中...** GPU 加速（SYCL）和剩余优化正在开发。

#### 功能列表

| 名称         | 类别 | 状态   | 模块版本 | 备注                                  |
| ------------ | ---- | ------ | -------- | ------------------------------------- |
| 地形生成优化 | 优化 | 已完成 | 26.1     | 没有改原版调度，理论上兼容所有地形MOD |
| 实体碰撞优化 | 优化 | 开发中 | 26.1     |                                       |
| 实体渲染优化 | 优化 | 计划中 | 26.1     |                                       |
| 实体AI优化   | 优化 | 开发中 | 26.1     |                                       |
...
- 本模组统一使用 `host` 内存，相较于 `device` 或 `shared` 内存可以省去拷贝到 `device` 的拷贝开销，因此更适合 `内存` 即 `显存` 的 `iGPU` 运行。现代 `iGPU` 通常挂在 `Ring Bus` 总线上而非 `PCIE` 连接并且自带大容量缓存，`iGPU` 可以通过 `Ring Bus` 总线直接访问 `内存` (部分型号带有 `L3` 、`L4` 缓存) 而无须拷贝。该方案因延迟问题与频繁拷贝问题不适合通过 `PCIE` 连接与传输的 `dGPU`。
